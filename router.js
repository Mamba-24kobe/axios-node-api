...
//form-data
router.post('/contact/add/form', koaBody, async (ctx) => {
...
router.post('/contact/add/json', async (ctx) => {
	let newData = ctx.request.body
	console.log(newData)
	id++
	newData.id = id
...
router.del('/contact/del', async (ctx) => {
...
router.get('/contact/delete', async (ctx) => {
	let id =getQueryVariable(ctx.request.url,'id')
	data = data.filter(item => item.id != id)
	console.log(id)
	ctx.body = {
